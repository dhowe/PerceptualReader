...
{	
  ...
  protected static Pattern patResult, patResult2, patNoResult;
...
//System.out.println("cookies: "+contents); 
    ...
    System.out.println("[INFO] Using Cookie: "+cStr);
    googleCookie = cStr;
  }
...
//System.out.println("NO RESULTS FOUND ***********************");     
      ...
      if (result >= 0) {
        ...
        new FileWriter("src/rita/google.html").write(htmlStr.toString());        
        throw new RiTaException
          ("No value found for query: '"+query+"' from google: html=\n"+html);
      }
    }
    catch (IOException e)
    {
...
trim = trim.replaceAll("\'", "\b");
if(trim.charAt(0) == '-' || trim.charAt(trim.length() - 1) == '-') trim = trim.replaceAll("-", "\b");
trim = trim.trim();
