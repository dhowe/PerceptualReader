// $Id: RiTa.java,v 1.5 2010/11/21 02:28:49 dhowe Exp $
 ...
 * 
 * $CVSHeader: RiTa/src/rita/RiTa.java,v 1.5 2010/11/21 02:28:49 dhowe Exp $
 */
...
public abstract class RiTa implements RiConstants
{
   ...
   */
  public static final String VERSION = "115";    
...
/*  *//**
   ...
   * @invisible
   *//*
  public static RiLerpBehavior startLerpBehavior(PApplet pApplet, float start, float target, float startOffsetSec, float durationSec) 
  {
    return (RiLerpBehavior)getTimerParent(pApplet).addBehavior
      (new RiLerpBehavior(null, start, target, startOffsetSec, durationSec));
  }*/

  /*private static RiText getTimerParent(Object pApplet)
  {
    if (timerParent == null) {
      if (!(pApplet instanceof PApplet)) {
        System.err.println("[WARN] The first argument" +
          " to this method is generally a PApplet subclass!\n");
      }
      else {
        timerParent = new RiText((PApplet)pApplet);
        timerParent.setVisible(false); // hidden
        timerParent.setAutoDraw(true);
        RiText.instances.remove(timerParent);
      }
    }
    return timerParent;
  }   */
...
//System.out.println("RiTa.mapToString("+m+") :: "+sb);
   ...
   * @param full String to be split
   * @param delim Delimiter string (regex)
   
  public static String[] split(CharSequence full, String delim)
  {
    return full.toString().split(delim);
  }*/
...
//System.out.println("RiTa.openStreamLocal("+streamName+")");
    ...
    for (int i = 0; i < guesses.length; i++) {
      ...
      if (!isDefaultFile) 
        ...
        if (!isDefaultFile) System.out.println("... OK");
    	} 
    	catch (FileNotFoundException e) {
        if (!isDefaultFile) System.out.println("... failed");
    	}
    	if (is != null) break;
    }
  ...
  // DOES THIS HANDLE UTF-8 correctly? test...
  ...
  /**
   * Same as PApplet.createInput but correctly handles UTF-8 characters.
  ...
  public static InputStream createInput(PApplet pApplet, String fileName)
  {
     ...
     if (is == null) 
       throw new RiTaException("The file '"+fileName+"' is missing or inaccessible, " +
          "make sure the URL is valid or that the file has been added to your data " +
        "folder and is readable.");
     return new UnicodeInputStream(is); // handles utf-8 properly...
  } */
  ...
  public static String[] loadStrings(PApplet pApplet, String fileName)
  {
    String[] lines = null;
    if (pApplet != null) {
      InputStream is = openStream(pApplet, fileName);
      if (is != null) {
        //UnicodeInputStream uis = new UnicodeInputStream(is);
        lines = PApplet.loadStrings(is);
      }
    }
    else //if (lines == null)  // uh-oh, who knows?
...
//System.err.println("openStream("+p+", "+fileName+")");
    ...
    try
    {
      is = (p != null) ? p.createInput(fileName) : openStreamLocal(fileName);
...
/*    
  private static String loadFile(PApplet p, String filename)
  {
    byte[] b = loadBytes(p, filename);
    return new String(b);
  }*/
...
//System.out.println("Props: "+props);
  ...
  {
    String word = "can't";
    System.out.println(RiTa.stripPunctuation(word));
    for (int i = 0; i < PUNCTUATION.length(); i++)
    {
      System.out.println(PUNCTUATION.charAt(i));
    }
    /*String[] sx = { " hello", "hello ", " hel'lo", "hel lo ", " 'hel\tlo'", "'hel'lo", };
    for (int i = 0; i < sx.length; i++)
    {
      System.out.println(i + ") '" + trimEnds(sx[i]) + "'");
    }*/
    /*String[] sx = { "gray.", "The", "U.S", "The", "boy;", "the", "boy?", "The"};
    for (int i = 0 ;  i< sx.length-1; i++)
    {
      System.out.println(i + ") " + isSentenceEnd(sx[i],sx[i+1]));
    }*/
