 ...
 *   Return RiTextBehavior's instead of Ids
 */
...
public class RiText extends RiObject implements RiCharSequence
{
  ...
  // protected static final float DEFAULT_FONT_SIZE_CONST = -1;
  protected static float DEFAULT_FONT_SIZE = -1;
  protected static final String DEFAULT_FONT_CONST = "ArialMT-12.vlw";

  // flag for default font creation: load(false) or create(true)
  protected static boolean CREATE_DEFAULT_FONT = false;
...
//      sample.update();
  ...
  {
    return this.moveBy(new float[] { xOffset, yOffset }, startTime, seconds);
  }
  ...
  public int moveBy(float[] posOffset, float startTime, float seconds)
  {
    ...
    float[] newPos = is3D(_pApplet.g) ? new float[3] : new float[2];
  ...
  /*  *//**
   * Clears the selected flag for all RiTexts
   * 
   * @invisible
   */
  /*
   * public static void clearSelected() { for (Iterator i =
   * instances.iterator(); i.hasNext();) { RiText rt = ((RiText) i.next());
   * rt.selected = false; } }
   */

  /*	*//**
   * Returns all selected RiTexts or null if none are selected
   * 
   * @return RiText[]
   * @invisible
   */
  /*
   * public static RiText[] getSelected() { List pts = null; for (Iterator i =
   * instances.iterator(); i.hasNext();) { RiText rt = ((RiText) i.next()); if
   * (rt.selected){ if (pts == null) pts = new LinkedList(); pts.add(rt); } } if
   * (pts == null || pts.size() == 0) return null; // just return null???
   * 
  ...
  protected static PFont _loadFont(PApplet pApplet, String fontFileName, float size)
  {
    ...
    if (pf == null)
    {
      ...
      if (pf == null)
      {
        ...
        {
          String errStr = "Could not load font '"
              + fontFileName
              + "'. Make "
   ...
   * @see #splitLetters()
   */
  public RiText[] split(PFont pfont, String regex, float yPos)
  {
  ...
  {
    // System.out.println("RiText.createLines("+text+", "+startX+", "+startY+", "+maxCharsPerLine+", "+
    // leading+")");
   ...
   * 'yStart'.
   */
  public static RiText[] createLetters(PApplet pApplet, PFont font, String theText, int startX, int startY)
  {
    RiText tmp = new RiText(pApplet, theText, startX, startY);
    if (font != null)
      tmp.textFont(font);
    RiText[] result = tmp.splitLetters();
    for (int i = 0; i < result.length; i++)
      result[i].textFont(font);
    delete(tmp);
    return result;
  }
  ...
  public RiText[] splitLetters()
  {
    ...
    {
      float mx = positionForChar(i);
      l.add(new RiText(_pApplet, chars[i], mx, y));
    }
