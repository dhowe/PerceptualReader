// $Id: RiSplitter.java,v 1.1 2010/10/30 23:09:04 dhowe Exp $
...
import rita.RiTa;
import rita.support.ifs.RiSplitterIF;

/**
 * A simple interface for different sentence splitters.<P>
 * Note: Adapted from JET & OAK
 */
public class RiSplitter implements RiSplitterIF
{  
  ...
  private static RiSplitter instance = null;
  
  private RiSplitter() {}
  
  /*public RiSplitter(PApplet p) {
    super(p);
    instance = this;
  }
...
/*    if (ignoreQuotedText) {
  ...
  private String clean(String sentence) {
    if (removeQuotations) {
      sentence = sentence.replaceAll("[\"”“]", ES);
      sentence = sentence.replaceAll("['`’‘] ", ES);
      sentence = sentence.replaceAll(" ['`’‘]", ES);
    }
    sentence = sentence.replaceAll("\\s+", SPC);
    return sentence;
  }
  ...
  private static final String SPC = " ", ES="";
  ...
  public static void main(String[] args)
  {
    ...
    txt = "The dog";
    RiSplitterIF sp = new RiSplitter();
